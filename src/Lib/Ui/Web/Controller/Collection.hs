...
import qualified Lib.Ui.Web.Page.CollectionOverview
                                               as CollectionOverviewPage
import qualified Lib.Ui.Web.Page.ShareCollectionOverview
                                               as ShareCollectionOverviewPage
...
collection = Route.CollectionSite
...
createCollection
  :: (CollectionRepo m, MonadRandom m, WithError m) => m Redirection
createCollection = do
...
createUnlockLink
  :: (CapabilityListRepo m, WithQuery env m)
  => CreateUnlockLinkForm
  -> m Redirection
 ...
 where
  mkCommand objRef colId = do
    let mPetname    = petname
        mExpDate    = unExpirationDate <$> expirationDate
        share       = False
        sharedPerms = mkOverviewPerms (isJust viewUnlockLinks)
                                      (isJust createUnlockLinks)
                                      (isJust delete)
                                      share
