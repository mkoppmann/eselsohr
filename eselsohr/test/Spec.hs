...
import Test.Core.Domain.Accesstoken (accToUrlRoundtrip, refToAccRoundtrip)
import Test.Hspec (Spec, hspec)
import Test.Hspec.Core.Spec (sequential)
import Test.Static (staticSpecs)
import UnliftIO.Directory (getTemporaryDirectory, removeDirectoryRecursive)

hspecTests :: AppEnv -> Spec
hspecTests =
  sequential . joinSpecs "Eselsohr" [staticSpecs]
...
main =
  bracket
    (loadConfig Nothing >>= setTmpDir >>= mkAppEnv)
