...
import qualified Network.HTTP.Client.Restricted
                                               as Client
import           Network.HTTP.Client.TLS        ( newTlsManagerWith
                                                , setGlobalManager
                                                )
import           Network.Socket                 ( AddrInfo(addrAddress)
                                                , SockAddr(..)
                                                , hostAddress6ToTuple
                                                , hostAddressToTuple
                                                )
...
useRestrictedHttpManager = liftIO $ setGlobalManager =<< restrictedManager
 where
  ...
  restrictedManager = do
    ...
    (settings, _) <- liftIO
      $ Client.mkRestrictedManagerSettings restriction Nothing Nothing
