...
createArticleForm = postMethodButton (fieldLink Route.createArticle) [urlInput] "Save article"
  where
    urlInput = input_ [type_ "url", name_ "articleUri", placeholder_ "URL"]

changeArticleTitleForm :: Id Article -> Text -> Accesstoken -> Link -> Html ()
changeArticleTitleForm artId artTitle =
    patchMethodButton
        (fieldLink Route.changeArticleTitle artId)
        [changeTitle]
        "Change title"
  where
    changeTitle = input_ [type_ "text", name_ "articleTitle", value_ $ toText artTitle]
...
markArticleAsUnreadForm artId = patchMethodLink (fieldLink Route.markArticleAsUnread artId) [unread] "Mark as unread"
  where
    unread = input_ [type_ "hidden", name_ "articleState", value_ "Unread"]
...
deleteSharedArticleListRefForm sharedArticleListIdRef =
    deleteMethodLink
